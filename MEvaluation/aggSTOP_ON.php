...
else if (is_array($array1) && is_array($array3)) {           
    foreach ($array1 as $one) {             //goes through meta array containg a result found in 2 engines but not all 3
         $Total_Documents = $Total_Documents +1;
        foreach ($array3 as $two) {
            if ($one["URL"] == $two["URL"]) {
                $No_Of_Rel_Docs = $No_Of_Rel_Docs + 1;
