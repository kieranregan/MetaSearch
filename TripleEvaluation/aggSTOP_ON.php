...
if(isset($BLEKKO->RESULT)){
foreach ($BLEKKO->RESULT as $value) {     //for each loop goes thur BLEKKO associative array
...
$mergedORIGINAL = array_merge($bordaENTIRE, $bordaBLEKKO, $bordaBING);     //merge the two arrays with scores that have not been added
    ...
    $array1 = remove_duplicates($mergedFINAL);

    uasort($array1, 'sort_scores');      //uasort uses the user defined function above to sort results by score
    $End = getTime();
...
if (is_array($array1) && is_array($array3)) {
    foreach ($array1 as $one) {         //goes through the meta search array for all 3 results if a result was found in all 3 
...
$recall = 0;
$recall = $No_Of_Rel_Docs/64;           //formula for totla recall

$AVG_PRECISION = $AVG_PRECISION/$No_Of_Rel_Docs;

mysql_query("INSERT INTO agg_stemon (query, total_documents, relevant, prec, recall, average_prec) 
